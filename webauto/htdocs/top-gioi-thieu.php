...
<h2 class="fw-light">TOP GIỚI THIỆU BẠN BÈ</h2>
<i class="fa-sharp fa-light fa-scarecrow"></i>
</section>
...
<table class="table table-bordered blueTable my-table">
    <thead>
        <tr>
            <th scope="fw-light">ID</th>
            <th scope="col">Tên Game</th>
            <th scope="col">Top GT</th>
        </tr>
    </thead>  
    <tbody>
    <?php 
...
$query = "SELECT username, SUM(gioithieu) AS gioithieu FROM `account` GROUP BY username ORDER BY gioithieu DESC LIMIT 100";
...
if ($result === false) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        <tr>
            <td>'.$stt.'</td>
            <td>'.$row['username'].'</td>
            <td>'.number_format($row['gioithieu']).'</td>
        </tr>
        ';
        $stt++;
    }
